...
test("cache-busts the stylesheet after tightening the chart header", () => {
  const html = fs.readFileSync(path.join(__dirname, "..", "index.html"), "utf8");

  assert.match(html, /style\.css\?v=20260830-chart-header-gap/);
});
...
test("cache-busts chart scripts after their latest behavior changes", () => {
  const html = fs.readFileSync(path.join(__dirname, "..", "index.html"), "utf8");
  ...
  for (const [asset, expectedVersion] of [
    ...
    ["script.js", "20260830-mag7-color-separation"],
  ]) {
    assert.match(
      html,
      new RegExp(`${asset.replace(".", "\\.")}\\?v=${expectedVersion}`),
